...
class CHWPSlowDAQTee(object):
    ...
    def _pack_for_slow_daq(self, data):
        ...
        buf = self._pack_uint(buf, data['chwp_encoder_quad'])
        buf = self._pack_uint(buf, data['chwp_encoder_clock'])
        buf = self._pack_uint(buf, data['chwp_encoder_count'])
        buf = self._pack_double(buf, data['chwp_irig_time'])
        buf = self._pack_uint(buf, data['chwp_irig_clock'])

        # Prepend buffer length to safeguard against incomplete
        # transmissions over the network
        # Replaced getsizeof() with len() to avoid the overhead generated by getsizeof() 2020-04-23 -HN
        buf = struct.pack(self._uint_str, len(buf)) + buf
        return buf
